...
class Postsold extends Component {
constructor(props){
 super(props);
...
<MDBCard className="my-5 px-2  pb-5">
      <MDBCardBody>
        <MDBRow>
          ...
          <MDBCol lg={(this.state.seeMoreImagesId!=post.id)?"12":"12"} xl={(this.state.seeMoreImagesId!=post.id)?"12":"12"}>
            {post.message&&
           ...
           <span className=" blue-dark-text"> {post.message.substring(0, 50)}...</span>
            ...
            <a color='red'  onClick={() =>{this.setState({readMoreImagesId:post.id})}}><span className=" font-weight-bold"> read more</span></a> 
          </>  }
            {((post.message.length>=200)&&(this.state.readMoreImagesId==post.id))&&<>
            {post.message}{"    "}
